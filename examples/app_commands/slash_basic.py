...
async def hello(ctx: pycord.ApplicationContext):
...
async def global_command(
    ctx: pycord.ApplicationContext, num: int
):  # Takes one integer parameter
    await ctx.respond(f"This is a global command, {num}!")


@bot.slash_command(guild_ids=[...])
async def joined(ctx: pycord.ApplicationContext, member: pycord.Member = None):
